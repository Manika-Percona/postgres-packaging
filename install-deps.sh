  ...
  pgbackrest)
    if [ "x$OS" = "xrpm" ]; then
      ...
      cd libbacktrace/
        ...
        make install
      cd ../
      INSTALL_LIST+="wget git vim rpm-build rpmdevtools libssh2-devel libcurl-devel lz4-libs libyaml-devel percona-postgresql${PG_MAJOR}-devel systemd systemd-devel bzip2-devel libxml2-devel openssl-devel perl perl-DBD-Pg perl-Digest-SHA perl-IO-Socket-SSL perl-JSON-PP zlib-devel gcc make autoconf perl-ExtUtils-Embed libssh-devel libzstd-devel lz4-devel meson libtool cmake"
      dnf -y install ${INSTALL_LIST}
      dnf -y install lz4 || true
      dnf -y install perl-libxml-perl || true
    else
      deb_deps
      INSTALL_LIST+="build-essential pkg-config liblz4-dev libssh2-1-dev debconf debhelper devscripts dh-exec libxml-checker-perl libxml-libxml-perl libio-socket-ssl-perl libperl-dev libssl-dev libxml2-dev txt2man zlib1g-dev libpq-dev percona-postgresql-${PG_MAJOR} percona-postgresql-common percona-postgresql-server-dev-all libbz2-dev libzstd-dev libyaml-dev meson python3-setuptools"
      DEBIAN_FRONTEND=noninteractive apt-get -y --allow-unauthenticated install ${INSTALL_LIST}
    fi
    ;;
